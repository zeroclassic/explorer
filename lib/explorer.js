...
module.exports = {
  ...
  get_supply: function(cb) {
    ...
    } else if (settings.supply == 'TXOUTSETZ') {
      var uri = base_url + 'gettxoutsetinfo';
      request({uri: uri, json: true}, function (error, response, body) {
        var total_transparent = body.total_amount;
        var uri_2 = base_url + 'getblockchaininfo';
        request({uri: uri_2, json: true}, function (error_2, response_2, body_2) {
          var total_sprout = body_2.valuePools[0].chainValue;
          var total_sapling = body_2.valuePools[1].chainValue;
          return cb(total_transparent + total_sprout + total_sapling);
        });
      });
    } else {
